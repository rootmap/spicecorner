                                        ...
                                        @if(isset($dataRow_Category))    
                                            @if(count($dataRow_Category)>0)
                                                @foreach($dataRow_Category as $Category)
                                                    <option value="{{$Category->id}}">{{$Category->name}} {{$Category->sub_name}}</option>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                  ...
                                  <select class="form-control select2" style="width: 100%;"  id="sub_category_id" name="sub_category_id">
                                        <option value="">Please Select</option>
...
@section("js")
    ...
    <script>
    var dataRow_SubCategory=<?php echo json_encode($dataRow_SubCategory); ?>;
    $(document).ready(function(){
        ...
        $("#category_id").change(function(){
            ...
            var htmlopt='<option value="">Please Select</option>';
            $.each(dataRow_SubCategory,function(key,row){
                if(row.category_id==category_id)
                {
                    htmlopt+='<option value="'+row.id+'">'+row.name+'</option>';
                }
            });

            $("#sub_category_id").html(htmlopt);
            $(".select2").select2();
